    ...
    chrome.storage.sync.get('token', function (data) {
        chrome.storage.sync.get('tok_exp', function (data2) {
            chrome.storage.sync.get('Pasted_url', function (data3) {
                chrome.storage.sync.get(['shiftStartTime','latecheckin','earlycheckout'], function (data4) {
                    if (data4['shiftStartTime']) {
                        $('#shifttime').text('Shift Start Time: '+data4.shiftStartTime);
                    }
                console.log(data4)
                var new_url = data.Pasted_url;
                new_url += "/api/auth/login"
                var proxyUrl = '';
                //loaction for late checkin
                var latitude;
                var longitude;
                ...
                // console.log(longitude)
                    ...
                    formData.append('reasonTitle', 'Late Checkin');
